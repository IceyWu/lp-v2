...
import { ViewerPro, type ViewerProOptions, type ImageObj } from 'viewer-pro'
...
export function useImageViewer(initialOptions: ViewerProOptions = {}) {
  ...
  const initViewer = useCallback(async () => {
    ...
    const viewerOptions: ViewerProOptions = {
      ...
      onImageLoad: initialOptions.onImageLoad || ((imgObj, idx) => {
  ...
  // 根据图片 URL 查找索引
  const findImageIndex = useCallback((src: string) => {
  ...
  const convertPostImagesToImageObj = useCallback((postImages: PostImage[]): ImageObj[] => {
  ...
  // 使用 PostImage 初始化查看器
  const initWithPostImages = useCallback((postImages: PostImage[]) => {
